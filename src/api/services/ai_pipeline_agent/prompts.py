...
Question: "Top 5 purchase orders by price"
Answer: [{{"$sort": {{"item.total_price": -1}}}}, {{"$limit": 5}}]

Question: "Top 5 purchase orders grouped by PO number"
Answer: [{{"$group": {{"_id": "$po_number", "total": {{"$sum": {{"$ifNull": ["$item.total_price", 0]}}}}}}}}, {{"$sort": {{"total": -1}}}}, {{"$limit": 5}}]

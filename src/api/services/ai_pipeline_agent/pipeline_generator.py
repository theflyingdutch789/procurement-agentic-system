...
class PipelineGenerator:
    ...
    def _supports_reasoning_effort(self) -> bool:
        """Check if the model supports reasoning effort parameter (GPT-5 series)."""
        return isinstance(self.model_name, str) and self.model_name.lower().startswith("gpt-5")

    def _supports_xhigh_reasoning(self) -> bool:
        """Check if the model supports xhigh reasoning effort (GPT-5.2+)."""
        model = self.model_name.lower() if isinstance(self.model_name, str) else ""
        return model.startswith("gpt-5.2") or model.startswith("gpt-5.3")
...
Return ONLY the JSON array, nothing else:"""
        ...
        # GPT-5.2 supports up to 128K output tokens, use 8K for pipeline generation
        max_tokens = 8000 if self._supports_xhigh_reasoning() else 4000
        ...
        output_text = extract_openai_response_text(response, logger=self.logger)
        if not output_text:
